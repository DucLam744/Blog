...
function BlogList() {
  ...
  }, [allBlogs, activeTag, itemsPerPage, searchQuery])
  ...
  const handleLike = async (blog) => {
    ...
    try {
      const userLiked = blog.likes.map((like) => String(like)).includes(String(currentUser.id))
      let updatedLikes

      if (userLiked) {
        updatedLikes = blog.likes.filter((id) => String(id) !== String(currentUser.id))
      } else {
      ...
      <div className="home-page">
        <div className="banner">
          <div className="container">
            <h1 className="logo-font">Blog</h1>
            ...
            <div className="col-md-9">
              ...
              {loading ? (
                ...
                displayedBlogs.map((item, index) => {
                  const hasLiked =
                    currentUser && item.likes && item.likes.map((id) => String(id)).includes(String(currentUser.id))
                  ...
                  return (
                    <div className="article-preview" key={index}>
                      <div className="article-meta">
                        ...
                        <div className="info">
                          <Link to={`/profile/${item.authorId}`} className="author">
